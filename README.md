...
一个 SillyTavern（酒馆）扩展：在每轮对话接收完整（含 MVU 变量解析）之后，自动把模型输出中 `<think>` / `<thinking>` 思考内容里的"危险 tag"去掉 `<>`（如 `<now_plot>` → `now_plot`），保留文字内容，防止酒馆助手 / 浏览器把未知 tag 当 HTML 解析而破坏渲染。
...
本插件在变量解析完成后，把这些 tag 的 `<>` 去掉（保留文字），让它们变成纯文本，从根源上消除渲染破坏，同时保留思考内容可读性与 `<think></think>` 折叠结构。
...
1. **触发时机**：
   - 检测到 **MVU**（MagVarUpdate）时，监听其 `VARIABLE_UPDATE_ENDED` 事件——这是"变量已落盘"的最精确信号，保证在 MVU 解析（含额外模型二次生成）之后再处理。
   - 未装 MVU 时回退到 `MESSAGE_RECEIVED` 事件立即处理；装了 MVU 时 `MESSAGE_RECEIVED` 作为延迟 2s 的兜底（覆盖无变量更新的消息）。
...
4. **边界标签保留**：`<think>` / `</think>` / `<thinking>` / `</thinking>` 本身不去 `<>`，原生 reasoning `auto_parse` 与预设自定义折叠仍能正常识别。
...
- **启用自动去 tag 化**：总开关
- **同时处理原生 reasoning (extra.reasoning)**：是否处理模型 API 返回的思考字段（非纯文本 `<think>` 场景）
- **危险 tag 白名单**：一行一个 tag 名。`think` / `thinking` 是边界标签，**不要加入**。可按你的预设增删。
- **立即处理全部消息**：手动处理当前聊天所有 AI 消息（用于插件首次启用后清理历史消息）
- 斜杠命令：`/detag-all` 同上
...
3. **仅收尾模式不补开标签**：初版只做"去 tag 化防崩"，不为仅收尾消息补 `<think>` 开标签（避免改变预设预期显示）。思考内容仍留在正文但不再破坏渲染。
4. **MVU 晚加载**：轮询等待 `window.Mvu` 最多 10s，超时仅用 `MESSAGE_RECEIVED` 兜底。
5. **编辑后的消息**：未监听 `MESSAGE_EDITED`，手动编辑加回的 tag 不会被自动处理，可用"立即处理全部消息"或 `/detag-all`。
...
测试覆盖 `detag` / `findThinkRegions` / `detagMes` / `detagReasoning` 的各种形态（开/闭/自闭合/带属性/中文 tag/幂等/标准配对/仅收尾/多块混合/无闭标签）。
